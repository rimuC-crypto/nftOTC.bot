...
async def safe_edit(q, text=None, caption=None, reply_markup=None, parse_mode="Markdown"):
    ...
    try:
        await q.edit_message_text(content, reply_markup=reply_markup, parse_mode=parse_mode)
    except Exception:
        try:
            await q.edit_message_caption(caption=content, reply_markup=reply_markup, parse_mode=parse_mode)
        except Exception:
            pass
...
async def main_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("confirm_cancel_"):
        ...
        deal = deals.get(did)
        if not deal:
            await q.edit_message_text(tr(ctx,"deal_not_found"), reply_markup=back_kb(ctx,"mydeals"), parse_mode="Markdown")
            ...
            save_data()
            await q.edit_message_text(tr(ctx,"deal_cancelled", id=did[:6]), reply_markup=back_kb(ctx,"mydeals"), parse_mode="Markdown")
        return MENU_ST
    ...
    if d == "req":
        ...
        await safe_edit(q,
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton(tr(ctx, "btn_req_balance"),  callback_data="req_balance")],
                [InlineKeyboardButton(tr(ctx, "back"),             callback_data="menu")],
            ]),
            parse_mode="Markdown"
        )
