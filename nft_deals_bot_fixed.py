...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8378557461:AAGEAO5D1-BcTd_drnvPWQlGitxM7Qet5aw")
...
# Через сколько минут автоматически снимать блокировку сделки
...
T["ru"] = {
    ...
    "btn_req_pay":      "💳 Привязать реквизиты",
    "btn_req_deposit":  "📥 Пополнить баланс",
    "btn_req_withdraw": "📤 Вывести средства",
    "btn_req_balance":  "💰 Мой баланс",
    "req_currencies_menu": (
        "💳 *Привязка реквизитов*\n\n"
    ...
    "deposit_pending": (
        ...
        "💰 Сумма: *{amount} {cur}*\n"
        "🆔 Заявка: `{req_id}`\n\n"
        "Свяжитесь с менеджером: @{support}\n"
        "Укажите ID заявки при обращении."
    ),
    "withdraw_menu": (
    ...
    "withdraw_enter": (
        "📤 *Вывод средств — {cur}*\n\n"
        "💰 Доступно: *{available} {cur}*\n\n"
        "Введите сумму вывода:\n"
        "_(Минимум: {min} {cur})_"
    ),
    "withdraw_no_req": (
        "⚠️ *Реквизиты не привязаны*\n\n"
        "Сначала привяжите реквизиты для валюты *{cur}*."
    ),
    ...
    "buyer_deal": (
        ...
        "После оплаты продавец обязан передать NFT.\n"
    ...
    "buyer_paid": (
        ...
        "✅ Продавец уведомлён.\n"
        "Как только он отправит NFT —\n"
        "вы получите уведомление.\n\n"
        "⚠️ Не отвечает более 30 минут? @{support}"
    ),
    "seller_notif": (
        "🎁 Подарок: *{gift}*\n"
        "💰 Сумма: *{amount} {cur}*\n"
        "👤 Покупатель: *{buyer}*\n"
        "🆔 ID: `{id}`\n\n"
        "👉 *Передайте NFT подарок и нажмите кнопку:*"
    ),
    ...
    "seller_waiting_confirm": (
        ...
        "⚠️ Покупатель не отвечает? — @{support}"
    ),
    "buyer_nft_received_prompt": (
        "🎁 Подарок: *{gift}*\n"
        "💰 Сумма: *{amount} {cur}*\n\n"
        "Проверьте, что NFT подарок получен.\n\n"
        "✅ Всё в порядке — подтвердите получение.\n"
        "⚖️ Подарок не получен — откройте спор."
    ),
    ...
    "deal_done_seller": (
        ...
        "Спасибо за использование OTC Market! 💎"
    ),
    "deal_done_buyer": (
...
T["uk"] = {
    ...
    "platform_stats": "🌐 *Статистика OTC Market*\n\n━━━━━━━━━━━━━━━━━━━━━━\n📋 Всього угод: *{total}*\n✅ Завершено: *{done}*\n📈 Успішність: *{pct}%*\n⚖️ Спорів: *{disputes}*\n👥 Користувачів: *{users}*\n━━━━━━━━━━━━━━━━━━━━━━",
    "req_menu": "💰 *Баланс:*\n{balance_lines}Оберіть дію:",
    "btn_req_pay": "💳 Прив'язати реквізити", "btn_req_deposit": "📥 Поповнити",
    "btn_req_withdraw": "📤 Вивести кошти", "btn_req_balance": "💰 Мій баланс",
    "deposit_menu": "Оберіть валюту поповнення:",
    "deposit_enter": "📥 *Поповнення — {cur}*\n\nВведіть суму в *{cur}*:\n\n_(Мінімум: {min} {cur})_",
    "deposit_pending": "⏳ *Заявку створено!*\n\n💰 Сума: *{amount} {cur}*\n🆔 Заявка: `{req_id}`\n\nЗв'яжіться: @{support}",
    "withdraw_menu": "Оберіть валюту для виведення:",
    "withdraw_enter": "📤 *Виведення — {cur}*\n\n💰 Доступно: *{available} {cur}*\n\nВведіть суму:\n_(Мінімум: {min} {cur})_",
    "withdraw_no_req": "⚠️ Реквізити не прив'язані для *{cur}*.",
    "withdraw_no_balance": "❌ Недостатньо коштів. Доступно: *{available} {cur}*",
    "withdraw_pending": "✅ *Заявку на виведення створено!*\n\n💰 {amount} {cur}\n📋 `{req}`\n🆔 `{req_id}`\n\n⏱ До 30 хвилин. @{support}",
    "balance_view": "{balance_lines}📅 На платформі з: *{joined}*\n⭐ Рейтинг: *{rating}* / 5.0\n📊 Угод: *{dc}*",
    "balance_empty": "  • Баланс порожній\n",
    "insufficient_balance": "❌ *Недостатньо коштів*\n\nПотрібно: *{need} {cur}*\nДоступно: *{have} {cur}*",
    "seller_balance_credited": "💰 *Кошти зараховано!*\n\n✅ Угода завершена\n💵 Зараховано: *{amount} {cur}*\n📊 Баланс: *{total} {cur}*",
    "nft_sent_already": "⚠️ NFT вже відмічено як переданий.",
    "confirm_receipt_already": "⚠️ Отримання вже підтверджено.",
    "profile_joined": "📅 На платформі з: *{date}*",
    "btn_platform_stats": "🌐 Статистика маркету",
    "btn_faq": "❓ FAQ",
    "buyer_timer_alert": (
        "⏰ *Час вийшов!*\n\n"
        "Минуло 30 хвилин, але продавець ще не передав NFT.\n\n"
        "🆔 `{id}`\n"
        "🎁 {gift}  |  💰 {amount} {cur}\n\n"
        "👇 Відкрийте спір:"
    ),
...
T["en"] = {
    ...
    "req_menu": "💰 *Balance:*\n{balance_lines}Choose action:",
    "btn_req_pay": "💳 Link Requisites", "btn_req_deposit": "📥 Deposit",
    "btn_req_withdraw": "📤 Withdraw", "btn_req_balance": "💰 My Balance",
    "deposit_menu": "Choose deposit currency:",
    "deposit_enter": "📥 *Deposit — {cur}*\n\nEnter amount in *{cur}*:\n\n_(Minimum: {min} {cur})_",
    "deposit_pending": "⏳ *Request created!*\n\n💰 Amount: *{amount} {cur}*\n🆔 Request: `{req_id}`\n\nContact: @{support}",
    "withdraw_menu": "Choose withdraw currency:",
    "withdraw_enter": "📤 *Withdraw — {cur}*\n\n💰 Available: *{available} {cur}*\n\nEnter amount:\n_(Minimum: {min} {cur})_",
    "withdraw_no_req": "⚠️ No requisites for *{cur}*. Add them first.",
    "withdraw_no_balance": "❌ Insufficient funds. Available: *{available} {cur}*",
    "withdraw_pending": "✅ *Withdrawal request created!*\n\n💰 {amount} {cur}\n📋 `{req}`\n🆔 `{req_id}`\n\n⏱ Up to 30 min. @{support}",
    "balance_view": "{balance_lines}📅 Member since: *{joined}*\n⭐ Rating: *{rating}* / 5.0\n📊 Deals: *{dc}*",
    "balance_empty": "  • Balance empty\n",
    "insufficient_balance": "❌ *Insufficient balance*\n\nNeed: *{need} {cur}*\nAvailable: *{have} {cur}*",
    "seller_balance_credited": "💰 *Funds credited!*\n\n✅ Deal completed\n💵 Credited: *{amount} {cur}*\n📊 Balance: *{total} {cur}*",
    "nft_sent_already": "⚠️ NFT already marked as sent.",
    "confirm_receipt_already": "⚠️ Receipt already confirmed.",
    "profile_joined": "📅 Member since: *{date}*",
    "btn_platform_stats": "🌐 Market Stats",
    "btn_faq": "❓ FAQ",
    "buyer_timer_alert": (
        "⏰ *Time is up!*\n\n"
        "30 minutes have passed but the seller hasn't sent the NFT.\n\n"
        "🆔 `{id}`\n"
        "🎁 {gift}  |  💰 {amount} {cur}\n\n"
...
T["ar"] = {
    ...
    "req_menu": "💰 *الرصيد:*\n{balance_lines}اختر إجراءً:",
    "btn_req_pay": "💳 ربط بيانات الدفع", "btn_req_deposit": "📥 إيداع",
    "btn_req_withdraw": "📤 سحب", "btn_req_balance": "💰 رصيدي",
    "deposit_menu": "اختر عملة الإيداع:",
    "deposit_enter": "📥 *إيداع — {cur}*\n\nأدخل المبلغ بـ *{cur}*:\n\n_(الحد الأدنى: {min} {cur})_",
    "deposit_pending": "⏳ *تم إنشاء الطلب!*\n\n💰 {amount} {cur}\n🆔 `{req_id}`\n\nتواصل: @{support}",
    "withdraw_menu": "اختر عملة السحب:",
    "withdraw_enter": "📤 *سحب — {cur}*\n\n💰 متاح: *{available} {cur}*\n\nأدخل المبلغ:\n_(الحد الأدنى: {min} {cur})_",
    "withdraw_no_req": "⚠️ لا توجد بيانات دفع لـ *{cur}*.",
    "withdraw_no_balance": "❌ رصيد غير كافٍ. المتاح: *{available} {cur}*",
    "withdraw_pending": "✅ *تم إنشاء طلب السحب!*\n\n💰 {amount} {cur}\n📋 `{req}`\n🆔 `{req_id}`\n\n⏱ حتى 30 دقيقة. @{support}",
    "balance_view": "{balance_lines}📅 عضو منذ: *{joined}*\n⭐ التقييم: *{rating}* / 5.0\n📊 الصفقات: *{dc}*",
    "balance_empty": "  • الرصيد فارغ\n",
    "insufficient_balance": "❌ *رصيد غير كافٍ*\n\nمطلوب: *{need} {cur}*\nمتاح: *{have} {cur}*",
    "seller_balance_credited": "💰 *تم إيداع الأموال!*\n\n✅ اكتملت الصفقة\n💵 مُودع: *{amount} {cur}*\n📊 الرصيد: *{total} {cur}*",
    "nft_sent_already": "⚠️ تم تحديد NFT كمُرسل بالفعل.",
    "confirm_receipt_already": "⚠️ تم تأكيد الاستلام بالفعل.",
    "profile_joined": "📅 عضو منذ: *{date}*",
    "btn_platform_stats": "🌐 إحصاء المنصة",
    "btn_faq": "❓ الأسئلة الشائعة",
    "platform_stats": "🌐 *إحصاء OTC Market*\n\n📋 إجمالي الصفقات: *{total}*\n✅ مكتملة: *{done}*\n📈 نسبة النجاح: *{pct}%*\n⚖️ نزاعات: *{disputes}*\n👥 مستخدمون: *{users}*",
    "faq": "❓ *الأسئلة الشائعة*\n\n🔐 *إذا لم يرسل البائع NFT؟*\nافتح نزاعاً — سيحله المشرف خلال 30 دقيقة.\n\n💰 *كيف يعمل الضمان؟*\nتُجمَّد الأموال حتى تأكيد استلام NFT.\n\n⏱ *كم يستغرق الأمر؟*\nيجب على البائع إرسال NFT خلال 30 دقيقة.\n\n💸 *ما هي العمولة؟*\n3٪ من المبلغ. تختار من يدفع.",
    "buyer_timer_alert": "⏰ انتهى الوقت! مرت 30 دقيقة والبائع لم يرسل NFT بعد.\n\n🆔 `{id}`\n🎁 {gift} | 💰 {amount} {cur}\n\n👇 افتح نزاعاً:",
}
...
T["zh"] = {
    ...
    "req_menu": "💰 *余额:*\n{balance_lines}选择操作:",
    "btn_req_pay": "💳 绑定收款信息", "btn_req_deposit": "📥 充值",
    "btn_req_withdraw": "📤 提现", "btn_req_balance": "💰 我的余额",
    "deposit_menu": "选择充值货币:",
    "deposit_enter": "📥 *充值 — {cur}*\n\n输入充值金额 *{cur}*:\n\n_(最低: {min} {cur})_",
    "deposit_pending": "⏳ *申请已创建!*\n\n💰 {amount} {cur}\n🆔 `{req_id}`\n\n联系: @{support}",
    "withdraw_menu": "选择提现货币:",
    "withdraw_enter": "📤 *提现 — {cur}*\n\n💰 可用: *{available} {cur}*\n\n输入金额:\n_(最低: {min} {cur})_",
    "withdraw_no_req": "⚠️ 未绑定 *{cur}* 收款信息。",
    "withdraw_no_balance": "❌ 余额不足。可用: *{available} {cur}*",
    "withdraw_pending": "✅ *提现申请已创建!*\n\n💰 {amount} {cur}\n📋 `{req}`\n🆔 `{req_id}`\n\n⏱ 最多30分钟。@{support}",
    "balance_view": "{balance_lines}📅 注册时间: *{joined}*\n⭐ 评分: *{rating}* / 5.0\n📊 交易: *{dc}*",
    "balance_empty": "  • 余额为空\n",
    "insufficient_balance": "❌ *余额不足*\n\n需要: *{need} {cur}*\n可用: *{have} {cur}*",
    "seller_balance_credited": "💰 *资金已到账!*\n\n✅ 交易完成\n💵 到账: *{amount} {cur}*\n📊 余额: *{total} {cur}*",
    "nft_sent_already": "⚠️ NFT已标记为已发送。",
    "confirm_receipt_already": "⚠️ 收货已确认。",
    "profile_joined": "📅 注册时间: *{date}*",
    "btn_platform_stats": "🌐 市场统计",
    "btn_faq": "❓ 常见问题",
    "platform_stats": "🌐 *OTC Market 统计*\n\n📋 总交易: *{total}*\n✅ 完成: *{done}*\n📈 成功率: *{pct}%*\n⚖️ 争议: *{disputes}*\n👥 用户: *{users}*",
    "faq": "❓ *常见问题*\n\n🔐 *如果卖家不发NFT？*\n发起争议 — 仲裁员30分钟内解决。\n\n💰 *托管如何运作？*\n资金冻结直到确认收到NFT。\n\n⏱ *交易需要多长时间？*\n卖家须在30分钟内发送NFT。\n\n💸 *手续费是多少？*\n交易额的3%，您选择谁来支付。",
    "buyer_timer_alert": "⏰ 时间到！已过30分钟，卖家尚未发送NFT。\n\n🆔 `{id}`\n🎁 {gift} | 💰 {amount} {cur}\n\n👇 建议发起争议:",
}
...
T["ja"] = {
    ...
    "req_menu": "💰 *残高:*\n{balance_lines}操作を選択:",
    "btn_req_pay": "💳 支払情報を登録", "btn_req_deposit": "📥 入金",
    "btn_req_withdraw": "📤 出金", "btn_req_balance": "💰 残高確認",
    "deposit_menu": "入金通貨を選択:",
    "deposit_enter": "📥 *入金 — {cur}*\n\n*{cur}*の金額を入力:\n\n_(最低: {min} {cur})_",
    "deposit_pending": "⏳ *申請を作成しました!*\n\n💰 {amount} {cur}\n🆔 `{req_id}`\n\n連絡先: @{support}",
    "withdraw_menu": "出金通貨を選択:",
    "withdraw_enter": "📤 *出金 — {cur}*\n\n💰 利用可能: *{available} {cur}*\n\n金額を入力:\n_(最低: {min} {cur})_",
    "withdraw_no_req": "⚠️ *{cur}* の支払情報が未登録です。",
    "withdraw_no_balance": "❌ 残高不足。利用可能: *{available} {cur}*",
    "withdraw_pending": "✅ *出金申請を作成しました!*\n\n💰 {amount} {cur}\n📋 `{req}`\n🆔 `{req_id}`\n\n⏱ 最大30分。@{support}",
    "balance_view": "{balance_lines}📅 登録日: *{joined}*\n⭐ 評価: *{rating}* / 5.0\n📊 取引: *{dc}*",
    "balance_empty": "  • 残高なし\n",
    "insufficient_balance": "❌ *残高不足*\n\n必要: *{need} {cur}*\n利用可能: *{have} {cur}*",
    "seller_balance_credited": "💰 *入金完了!*\n\n✅ 取引完了\n💵 入金: *{amount} {cur}*\n📊 残高: *{total} {cur}*",
    "nft_sent_already": "⚠️ NFTは既に送信済みとしてマークされています。",
    "confirm_receipt_already": "⚠️ 受取は既に確認済みです。",
    "profile_joined": "📅 登録日: *{date}*",
    "btn_platform_stats": "🌐 市場統計",
    "btn_faq": "❓ よくある質問",
    "platform_stats": "🌐 *OTC Market 統計*\n\n📋 総取引数: *{total}*\n✅ 完了: *{done}*\n📈 成功率: *{pct}%*\n⚖️ 紛争: *{disputes}*\n👥 ユーザー: *{users}*",
    "faq": "❓ *よくある質問*\n\n🔐 *売り手がNFTを送らない場合？*\n異議申し立てを開始 — 仲裁人が30分以内に解決します。\n\n💰 *エスクローの仕組みは？*\nNFT受取確認まで資金は凍結されます。\n\n⏱ *取引にかかる時間は？*\n売り手は30分以内にNFTを送る必要があります。\n\n💸 *手数料は？*\n取引額の3%。誰が支払うかを選択します。",
    "buyer_timer_alert": "⏰ 時間切れ！30分が経過しましたが、売り手はまだNFTを送っていません。\n\n🆔 `{id}`\n🎁 {gift} | 💰 {amount} {cur}\n\n👇 異議申し立てを開始:",
}
...
async def safe_edit(q, text=None, caption=None, reply_markup=None, parse_mode="Markdown"):
    """Безопасное редактирование — работает и для текстовых и для фото-сообщений."""
    msg = text or caption or ""
    try:
        await q.edit_message_text(msg, reply_markup=reply_markup, parse_mode=parse_mode)
    except Exception:
        try:
            await q.edit_message_caption(caption=msg, reply_markup=reply_markup, parse_mode=parse_mode)
        except Exception:
            pass
...
async def adm_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "adm_stats":
        ...
        if not active_deals:
            await safe_edit(q, "📭 Нет активных сделок.", reply_markup=back_adm)
            return
        kb = []
        for did, dl in active_deals[:15]:
            kb.append([InlineKeyboardButton(
                f"🟡 #{did[:6]} — {dl['gift']} | {dl['amount']} {dl['currency']}",
        ...
        deal = deals.get(did)
        if not deal:
            await safe_edit(q, "❌ Сделка не найдена.", reply_markup=back_adm)
            return
        if deal["status"] != "active":
            await safe_edit(q, f"⚠️ Статус: {deal['status']}", reply_markup=back_adm)
            return
        deal["status"]     = "paid"
        deal["buyer_id"]   = None
        ...
        try:
            await ctx.bot.send_message(
                ...
                text=tr_raw(sl,"seller_notif",
                            ...
                            buyer=deal["buyer_name"], id=did[:6]),
        ...
        await safe_edit(q, "🚫 Введите ID пользователя:", reply_markup=back_adm, parse_mode="Markdown")

    elif d == "adm_rating":
        ctx.user_data["adm_action"] = "rating"
        await safe_edit(q, "⭐ Формат: `ID значение`\nПример: `123456789 4.9`", reply_markup=back_adm, parse_mode="Markdown")

    elif d == "adm_dc":
        ctx.user_data["adm_action"] = "deals_count"
        await safe_edit(q, "📊 Формат: `ID количество`", reply_markup=back_adm, parse_mode="Markdown")

    elif d == "adm_bal":
        ctx.user_data["adm_action"] = "balance"
        curs = " | ".join(CURRENCIES.keys())
        await safe_edit(q, f"💰 Формат: `ID ВАЛЮТА сумма`\nВалюты: `{curs}`", reply_markup=back_adm, parse_mode="Markdown")

    elif d == "adm_users":
        if not users:
            await safe_edit(q, "📭 Нет пользователей.", reply_markup=back_adm)
            return
        txt = "👥 *Пользователи:*\n\n"
        ...
        await safe_edit(q, txt, reply_markup=back_adm, parse_mode="Markdown")

    elif d == "adm_alldeals":
        if not deals:
            await safe_edit(q, "📭 Нет сделок.", reply_markup=back_adm)
            return
        txt = "📋 *Все сделки:*\n\n"
        for did, dl in list(deals.items())[:20]:
            txt += f"{status_icon(dl['status'])} `{did[:6]}` {dl['gift']} | {dl['amount']} {dl['currency']}\n"
        await safe_edit(q, txt, reply_markup=back_adm, parse_mode="Markdown")

    elif d == "adm_back":
        await safe_edit(q, tr_raw("ru","admin_granted"), reply_markup=admin_kb(), parse_mode="Markdown")

    elif d == "adm_close":
        ctx.user_data.pop("is_admin", None)
        await safe_edit(q, "✅ Панель закрыта.")
...
async def main_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in blocked_users and not d.startswith("adm_"):
        lang = ctx.user_data.get("lang","ru")
        await safe_edit(q, tr_raw(lang,"blocked"), parse_mode="Markdown")
        return MENU_ST
    ...
    if d.startswith("lang_"):
        ...
        if ctx.user_data.get("flow") == "buyer":
            ...
            if not deal or deal["status"] in ("done","cancelled"):
                await safe_edit(q, tr(ctx,"deal_not_found"), parse_mode="Markdown")
                return MENU_ST
        ...
        WELCOME_PHOTO_ID = ""  # Disabled - causes edit_message_text to fail on all buttons
        sent = False
        if WELCOME_PHOTO_ID:
            try:
    ...
    if d == "create":
        ...
        await safe_edit(q, tr(ctx,"deal_cur"), reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
        return DEAL_CUR_ST
            ...
            for did in reversed(u["deals"][-20:]):
                ...
                if dl:
                    rows += tr(ctx,"deal_row", icon=status_icon(dl["status"]),
                               id=did[:6], gift=dl["gift"], amount=dl["amount"], cur=dl["currency"])
                    if dl["status"] == "active":
                        active_deals_kb.append([InlineKeyboardButton(
                            f"🗑 Отменить #{did[:6]}", callback_data=f"cancel_deal_{did}"
                        )])
                    detail_kb.append([InlineKeyboardButton(
                        f"🔎 #{did[:6]} {dl['gift'][:15]}", callback_data=f"detail_{did}"
                    )])
            ...
            for did2 in reversed(buyer_deals[-10:]):
                ...
                if dl2:
                    brows += tr(ctx,"deal_row", icon=status_icon(dl2["status"]),
                                id=did2[:6], gift=dl2["gift"], amount=dl2["amount"], cur=dl2["currency"])
            if brows:
                buyer_section = "\n🛒 *История покупок:*\n" + brows
    ...
    if d.startswith("cancel_deal_"):
        ...
        if deal["seller_id"] != uid:
            ...
            return MENU_ST
        await safe_edit(q, 
            tr(ctx,"deal_cancel_confirm", id=did[:6], gift=deal["gift"],
    ...
    if d.startswith("confirm_cancel_"):
        ...
        deal = deals.get(did)
        if not deal:
            await safe_edit(q, tr(ctx,"deal_not_found"), reply_markup=back_kb(ctx,"mydeals"), parse_mode="Markdown")
        elif deal["status"] in ("paid","done","nft_sent"):
            await safe_edit(q, tr(ctx,"deal_cancel_fail"), reply_markup=back_kb(ctx,"mydeals"), parse_mode="Markdown")
        else:
            deal["status"] = "cancelled"
            save_data()
            await safe_edit(q, tr(ctx,"deal_cancelled", id=did[:6]), reply_markup=back_kb(ctx,"mydeals"), parse_mode="Markdown")
        return MENU_ST
    ...
    if d == "req":
        ...
        kb.append([InlineKeyboardButton(tr(ctx, "back"), callback_data="menu")])
        req_list = ""
        for code, cname in CURRENCIES.items():
            if code in u["req"]:
                req_list += f"✅ {cname}: `{u['req'][code]}`\n"
        header = ("💳 *Привязанные реквизиты:*\n" + req_list + "\n") if req_list else ""
        header += "💳 Выберите валюту для привязки:"
        await safe_edit(q, header, reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
        return REQ_CUR_ST
        ...
        for i in range(0, len(items), 2):
            ...
            for j in range(2):
                if i+j < len(items):
                    ...
                    row.append(InlineKeyboardButton(f"{check}{cname}", callback_data=f"rq_{code}"))
            kb.append(row)
        kb.append([InlineKeyboardButton(tr(ctx, "back"), callback_data="req")])
        await safe_edit(q, 
            tr(ctx, "req_currencies_menu"),
        ...
        await safe_edit(q, 
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(tr(ctx, "btn_req_deposit"),  callback_data="req_deposit"),
                 InlineKeyboardButton(tr(ctx, "btn_req_withdraw"), callback_data="req_withdraw")],
                [InlineKeyboardButton(tr(ctx, "back"), callback_data="req")],
            ]),
            parse_mode="Markdown"
        )
        return REQ_CUR_ST

    # ── Deposit: choose currency ──────────────────
    if d == "req_deposit":
        items = list(CURRENCIES.items())
        kb = []
        for i in range(0, len(items), 2):
            ...
            for j in range(2):
                if i+j < len(items):
                    code, cname = items[i+j]
                    row.append(InlineKeyboardButton(cname, callback_data=f"dep_cur_{code}"))
            kb.append(row)
        kb.append([InlineKeyboardButton(tr(ctx, "back"), callback_data="req")])
        await safe_edit(q, 
            tr(ctx, "deposit_menu"),
    ...
    if d.startswith("dep_cur_"):
        cur = d[8:]
        if cur in CURRENCIES:
            ctx.user_data["dep_cur"] = cur
            ctx.user_data["await"]   = "deposit"
            min_val = MIN_AMOUNTS.get(cur, 1)
            await safe_edit(q, 
                tr(ctx, "deposit_enter", cur=CURRENCIES.get(cur, cur), min=min_val),
                reply_markup=back_kb(ctx, "req_deposit"),
                parse_mode="Markdown"
            )
            return DEPOSIT_ST
        return MENU_ST

    # ── Withdraw: choose currency ─────────────────
    if d == "req_withdraw":
        items = list(CURRENCIES.items())
        kb = []
        for i in range(0, len(items), 2):
            ...
            for j in range(2):
                if i+j < len(items):
                    code, cname = items[i+j]
                    bal_val = u.get("balance", {}).get(code, 0)
                    row.append(InlineKeyboardButton(
                        f"{cname} ({bal_val})", callback_data=f"wdr_cur_{code}"
                    ))
            kb.append(row)
        kb.append([InlineKeyboardButton(tr(ctx, "back"), callback_data="req")])
        await safe_edit(q, 
            tr(ctx, "withdraw_menu"),
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
        return WITHDRAW_ST

    if d.startswith("wdr_cur_"):
        cur = d[8:]
        if cur in CURRENCIES:
            available = u.get("balance", {}).get(cur, 0)
            req_val   = u.get("req", {}).get(cur)
            if not req_val:
                await safe_edit(q, 
                    tr(ctx, "withdraw_no_req", cur=CURRENCIES.get(cur, cur)),
                    reply_markup=back_kb(ctx, "req_withdraw"),
                    parse_mode="Markdown"
                )
                return WITHDRAW_ST
            min_val = MIN_AMOUNTS.get(cur, 1)
            if available < min_val:
                await safe_edit(q, 
                    tr(ctx, "withdraw_no_balance", available=available,
                       cur=CURRENCIES.get(cur, cur)),
                    reply_markup=back_kb(ctx, "req_withdraw"),
                    parse_mode="Markdown"
                )
                return WITHDRAW_ST
            ctx.user_data["wdr_cur"]  = cur
            ctx.user_data["await"]    = "withdraw"
            await safe_edit(q, 
                tr(ctx, "withdraw_enter",
                   cur=CURRENCIES.get(cur, cur),
                   available=available,
                   min=min_val),
                reply_markup=back_kb(ctx, "req_withdraw"),
    ...
    if d == "changelang":
        try:
            await q.edit_message_caption(caption=tr(ctx,"lang_pick"), reply_markup=lang_kb(), parse_mode="Markdown")
        except Exception:
            try:
                await safe_edit(q, tr(ctx,"lang_pick"), reply_markup=lang_kb(), parse_mode="Markdown")
            except Exception:
                pass
        return LANG_ST
    ...
    if d == "support":
        ...
        await safe_edit(q, tr(ctx,"support"), reply_markup=back_kb(ctx))
        return SUPPORT_ST

    if d == "about":
        await safe_edit(q, tr(ctx,"about"), reply_markup=back_kb(ctx))
        return MENU_ST

    # ── FAQ ───────────────────────────────────────
    if d == "faq":
        await safe_edit(q, tr(ctx,"faq"), reply_markup=back_kb(ctx))
        return MENU_ST
    ...
    if d.startswith("detail_"):
        ...
        if not deal:
            ...
            return MENU_ST
        snames = {
            "active":    tr(ctx,"s_active"),
            "paid":      tr(ctx,"s_paid"),
            "nft_sent":  tr(ctx,"s_nft_sent"),
            "done":      tr(ctx,"s_done"),
            "cancelled": tr(ctx,"s_cancelled"),
        }
        buyer_line = f"👤 {deal['buyer_name']}\n" if deal.get("buyer_name") else ""
        await safe_edit(q,
            tr(ctx,"deal_detail", id=did[:6], gift=deal["gift"],
    ...
    if d == "search_deal":
        ...
        await safe_edit(q, tr(ctx,"search_deal_prompt"), reply_markup=back_kb(ctx,"mydeals"), parse_mode="Markdown")
        return MENU_ST
    ...
    if d.startswith("pay_"):
        ...
        if deal.get("locked_by") and deal["locked_by"] != uid:
            ...
            return BUYER_ST
        deal["locked_by"]  = uid
        deal["lock_time"]  = datetime.now().strftime("%d.%m.%Y %H:%M")
    ...
    if d.startswith("dispute_"):
        ...
        save_data()
        await safe_edit(q, 
            tr(ctx,"dispute_opened", id=did[:6]),
        ...
        try:
            await ctx.bot.send_message(
                chat_id=deal["seller_id"],
                text=tr_raw(sl,"dispute_notif", id=did[:6],
        ...
        if did:
            ...
            if deal and deal.get("status") == "active":
                ...
                try:
                    await ctx.bot.send_message(
                        chat_id=deal["seller_id"],
                        text=tr_raw(sl,"buyer_left", id=did[:6],
                                    gift=deal["gift"], amount=deal["amount"],
                                    cur=CURRENCIES.get(deal["currency"],deal["currency"])),
                        parse_mode="Markdown"
                    )
...
async def msg_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if aw == "deposit":
        try:
            ...
            cur     = ctx.user_data.get("dep_cur", "USDT")
            min_val = MIN_AMOUNTS.get(cur, 1)
            if amount < min_val:
                await update.message.reply_text(
                ...
                return DEPOSIT_ST
            req_id = str(uuid.uuid4()).replace("-", "")[:8].upper()
            ctx.user_data.pop("await",   None)
            ctx.user_data.pop("dep_cur", None)
            # Notify support
            try:
                await ctx.bot.send_message(
            ...
            save_data()
            await update.message.reply_text(
                tr(ctx, "deposit_pending", amount=amount,
                   cur=CURRENCIES.get(cur, cur), req_id=req_id),
                reply_markup=menu_kb(ctx),
                parse_mode="Markdown"
            )
        except ValueError:
    ...
    if aw == "withdraw":
        try:
            amount    = float(text.replace(",", ".").replace(" ", ""))
            cur       = ctx.user_data.get("wdr_cur", "USDT")
            available = u.get("balance", {}).get(cur, 0)
            min_val   = MIN_AMOUNTS.get(cur, 1)
            if amount < min_val:
                await update.message.reply_text(
            ...
            req_val = u.get("req", {}).get(cur, "—")
            # Deduct balance
            u.setdefault("balance", {})[cur] = round(available - amount, 8)
            save_data()
            ctx.user_data.pop("await",   None)
            ctx.user_data.pop("wdr_cur", None)
            # Notify support
            try:
                await ctx.bot.send_message(
                    chat_id=f"@{SUPPORT_USERNAME}",
                ...
                pass
            await update.message.reply_text(
                tr(ctx, "withdraw_pending", amount=amount,
                   cur=CURRENCIES.get(cur, cur), req=req_val, req_id=req_id),
                reply_markup=menu_kb(ctx),
                parse_mode="Markdown"
            )
        except ValueError:
    ...
    if aw == "gift":
        ...
        did      = str(uuid.uuid4()).replace("-","")[:6].upper()
        bot_info = await ctx.bot.get_me()
        deals[did] = {
            "seller_id":   uid,
...
def main():
    ...
    # ── Авто-снятие блокировки зависших сделок ────────────────────
    async def unlock_stale_deals_job(context):
        """Каждые 5 минут проверяет сделки с lock_time и снимает блокировку."""
        now = datetime.now()
        count = 0
    ...
    print("✅ OTC Market Bot v4.1 running...")
